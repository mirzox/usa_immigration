<template>
    ...
    <div class="winds hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full"
        :id="id + 'forms_modal'" tabindex="-1" aria-hidden="true">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="close_windows" :data-modal-hide="id + 'forms_modal'">
            ...
            <div class="relative bg-white">
                <form class="max-w-sm mx-auto" @submit.prevent="sendMessage">
                    ...
                    <button :data-modal-hide="id + 'forms_modal'" type="submit" class="checls"> {{$t('interface.form.inputs.button_text')}} </button>
                </form>
            </div>
        </div>
    </div>
    ...
    async sendMessage() {
        try {
            ...
            if (this.name && this.phoneNumber) {
                const MESSAGE_TEXT = `
<b>Услуга</b>: ${this.$i18n.locale == 'ru-RU' ? this.data.title_ru : this.data.title_en }
<b>Имя</b>: ${this.name}
